...
- 上传图片会自动保存到项目本地资产目录，避免 scene 里长期保存大段 base64。
...
画布左上角会显示一个“任务”按钮，默认不展开，避免遮挡画布操作。点击后会打开“任务识别”面板，用来提前确认 Codex 会怎样理解当前画布：

- 当前模式：单图锁定、占位锁定、多图批量、多图选区、单图选中或等待图片。
- 图片数量：当前画布里可作为修改目标的图片数量。
- 任务数量：Canvaswright 已经从圈选、箭头、文字里识别出的修改任务数量。
- 疑问数量：无法明确归属到某张图的标注数量，需要把标注移近对应图片或取消跨多图圈选。
- 任务列表：展示每张目标图对应的标注数量和文字说明摘要。

如果面板显示“单图锁定”，Codex 会优先只改锁定的那张图。要批量修改多张图，先取消锁定目标，再分别给每张图画标注。
